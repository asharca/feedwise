...
import { Search, Rss, Hash, BookOpen, Inbox } from "lucide-react";
 ...
 * (Articles + Feeds + Tags sections with snippets) and adds a wider
 * two-pane grid so the user can browse the chronological article tail
 * alongside the relevance top-20 from `/api/search`.
 *
...
export function SearchResultsPage({
  ...
  return (
    <div className="flex h-full">
      <div className="flex flex-col border-r border-border bg-background shrink-0 md:w-[28rem]">
            ...
            <div className="flex flex-col">
              ...
              {data && data.articles.length === 0 && (data.feeds.length > 0 || data.tags.length > 0) && (
                <div className="px-3 py-2 text-[11px] text-muted-foreground/80 border-b border-border/50">
                  ...
                  match{data.feeds.length + data.tags.length === 1 ? "" : "es"} below.
                </div>
              )}
      ...
      {/* Right rail: Feeds + Tags */}
      <div className="hidden lg:flex flex-col w-72 shrink-0 p-4 gap-6 overflow-y-auto scrollbar-thin">
        <RailSection
          label="Feeds"
          count={feedCount}
          empty={!loading && feedCount === 0}
        >
          {data?.feeds.map((f) => (
            <button
              key={f.feedId}
              ...
              className="w-full flex items-center gap-2 py-1.5 px-2 rounded hover:bg-accent text-sm text-left transition-colors"
            >
              {f.iconUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
