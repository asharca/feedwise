...
The plan originally called for replacing the existing inline "Preview email" dialog in `digest-email-section.tsx` (which previews the _next_ digest via `/api/email/llm/preview`) with the new `EmailPreviewDialog` (which fetches a _past_ digest via `/api/settings/email/history/[logId]/preview`). The two components have different fetch semantics and the spec's "no behavior change" claim was incorrect.
